...
### 13. Pongamos un ejemplo de dos alternativas para lo mismo. Tenemos un `Estudiante` y un `Trabajador`, ambos tienen datos en común: el DNI y el nombre. 
#### Modelemos esto de dos formas: uno por herencia, con una superclase `Persona`,
...
class EstudianteH extends Persona {
    public EstudianteH(String dni, String nombre) {
        super(dni, nombre);
    }
}
class TrabajadorH extends Persona {
    public TrabajadorH(String dni, String nombre) {
        super(dni, nombre);
    }
}
```
#### y otro con composición, con una clase `DatosPersonales`. Se debe recibir una instancia de `DatosPersonales` en el constructor de la clase `Estudiante` y `Trabajador`.
```java
